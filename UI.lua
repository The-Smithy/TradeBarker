...
function TB.RebuildPresetList()
    ...
    for i, preset in ipairs(presets) do
        local idx = i  -- explicit local capture for all closures in this iteration
        ...
        nameBox:SetScript("OnEnterPressed", function(self)
            TB.RenamePreset(idx, self:GetText())
            self:ClearFocus()
        ...
        -- Update button (overwrite preset with current selections)
        local updBtn = TB.CreateFlatButton(row, "Update", 44)
        updBtn:SetHeight(20)
        updBtn:SetPoint("LEFT", delBtn, "RIGHT", 2, 0)
        updBtn:SetBackdropColor(0.1, 0.12, 0.2, 0.9)
        updBtn:SetBackdropBorderColor(0.3, 0.4, 0.7, 0.8)
        updBtn:GetFontString():SetTextColor(0.5, 0.7, 1)
        updBtn:SetScript("OnEnter", function(self)
            self:SetBackdropColor(0.15, 0.18, 0.3, 0.9)
            self:SetBackdropBorderColor(0.4, 0.6, 1, 0.8)
            self:GetFontString():SetTextColor(0.7, 0.85, 1)
            GameTooltip:SetOwner(self, "ANCHOR_TOP")
            GameTooltip:SetText("Update Preset", 1, 1, 1)
            GameTooltip:AddLine("Overwrite this preset with the currently checked items.", nil, nil, nil, true)
            GameTooltip:Show()
        end)
        updBtn:SetScript("OnLeave", function(self)
            self:SetBackdropColor(0.1, 0.12, 0.2, 0.9)
            self:SetBackdropBorderColor(0.3, 0.4, 0.7, 0.8)
            self:GetFontString():SetTextColor(0.5, 0.7, 1)
            GameTooltip:Hide()
        end)
        ...
        upBtn:SetHeight(20)
        upBtn:SetPoint("LEFT", updBtn, "RIGHT", 2, 0)
        if i == 1 then
            upBtn:SetAlpha(0.3)
            upBtn:Disable()
        end
        upBtn:SetScript("OnClick", function()
            TB.MovePreset(idx, "up")
            TB.RebuildPresetList()
        end)
